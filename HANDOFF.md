...
- **CAR bulk edit + กรองตามผู้รับผิดชอบ (25 JUL 2026):** เพิ่มฟิลด์ `c.idate` = วันที่มอบหมายรายใบ (เดิมฟอร์มใช้ meta.date ของทั้งรอบ) แสดงเป็นคอลัมน์ "มอบหมาย" ในตาราง CAR และใช้ในช่อง Date issued ของฟอร์ม D-0507-CAR-001 · แผง "✏️ แก้ไขหลายใบพร้อมกัน (Bulk edit)" — ตั้งวันที่มอบหมาย/กำหนดเสร็จ/ผู้รับผิดชอบ/อีเมล/ตำแหน่ง/Finding level ให้ CAR ที่เลือกพร้อมกัน (ช่องที่เว้นว่าง = ไม่แก้ · confirm สรุปการเปลี่ยนก่อนบันทึก · ลง history log ทุกใบ) ใช้ปรับระบบให้ตรงกับเอกสารกระดาษที่มอบหมายไปก่อนแล้ว · ตัวกรอง: ผู้รับผิดชอบ (dropdown พร้อมจำนวนใบต่อคน) + สถานะ (ยังไม่ปิด/เกินกำหนด/รอแก้ไข/รอตรวจรับ/ตีกลับ/ปิดแล้ว) + ปุ่มเลือกทั้งหมดที่กรอง/ล้างที่เลือก

- **CAR select-all + bulk sign 3 บทบาท (25 JUL 2026):** checkbox ที่หัวตาราง CAR เลือก/ยกเลิกทุกใบที่แสดงอยู่ (สอดคล้องกับตัวกรอง) · ปุ่มเซ็นครบ 3 บทบาท "✍ ผู้รับผิดชอบ / Auditor / CMM เซ็นที่เลือก" — เซ็นครั้งเดียว apply ทุกใบที่เลือกโดยไม่เปลี่ยนสถานะ (แยกจากปุ่มส่งตอบกลับ/ตรวจรับที่เปลี่ยนสถานะด้วย) · ทดสอบแล้ว: กด checkbox หัวตาราง → เลือกครบ 45 ใบ
- **ร่างคำตอบ CAR ทั้งชุด (25 JUL 2026):** กรอก Part 3–5 (root cause / corrective + วันแล้วเสร็จ / preventive) ให้ CAR ทุกใบของรอบ iac-2026-01 ผ่านสคริปต์ในหน้าเว็บ แยกเนื้อหาตามประเภท (TM มีสื่อ 36 · TM ต้องสร้างสื่อใหม่ 13 · OMM SMS · SITE อากาศยาน/FRAE) วันแล้วเสร็จ default 30 ก.ย. 2569 · **ยังไม่ได้กดส่ง/เซ็น** — สถานะยัง "รอแก้ไข" รอผู้รับผิดชอบตรวจทานและลงนามเอง
...
## 26 SEP 2026 — แก้ปัญหา "กรอกข้อความไม่ได้" · SM ปิดงานนิรภัย · ช่องกรอกมีคำแนะนำ+ตัวอย่าง
- **สาเหตุที่คนอื่นกรอกไม่ได้ (ที่เจอในโค้ด):** ลิงก์ `👁 เปิดดูรายการตรวจ` ในหน้าคลังเช็กลิสต์พาไป `?view=1` ซึ่ง CSS `body.viewonly` หรี่และปิดการคลิกทุก input — เดิมแถบบอกสถานะขึ้นเฉพาะตอน**ยังไม่ล็อกอิน** คนที่ล็อกอินอยู่แล้วจึงเห็นหน้าปกติแต่พิมพ์ไม่ได้ โดยไม่มีอะไรบอก · แก้เป็นขึ้นแถบเหลืองเสมอเมื่อมี `?view=1` พร้อมปุ่ม **✏ เข้าโหมดกรอกข้อมูล** (ตัด `view` ออกจาก URL)
- เพิ่มแถบแดงเมื่อ Firestore ตอบ `permission-denied` (ทั้งตอนอ่านและตอนเขียน) บอกอีเมลที่ใช้อยู่ รอบตรวจ และให้กด 💾 JSON เก็บก่อน — เดิมขึ้นเป็นข้อความเล็กท้ายจอว่า "จะลองใหม่" แล้ววนเงียบ ๆ
- **ยังไม่ได้ยืนยันว่าเป็นสาเหตุเดียว** — ถ้าผู้ใช้รายนั้นยังกรอกไม่ได้ ให้ดูว่าแถบไหนขึ้น (เหลือง = โหมดอ่าน · แดง = สิทธิ์) แล้วแจ้งกลับ
- **SM ปิดงานตรวจด้านนิรภัย:** Portal เพิ่มช่องอีเมล Safety Manager (เก็บที่ `plans/{year}.sm`) · `closeRound()` แทน checkbox เดิม — รอบ project `safety`/`surveillance` ปิดได้เฉพาะบัญชี SM · ต้องผ่าน ACM อนุมัติก่อน · เตือนถ้าตรวจไม่ครบหรือมี CAR ค้าง · บันทึก `r.cl={n,by,dt}` และแสดง "🔒 ชื่อ · วันที่" ใต้สถานะ · เปิดงานกลับต้องยืนยันและต้องเป็น SM เช่นกัน
- **ช่องกรอกทุกช่องหลัก** เปลี่ยน placeholder เป็นรูปแบบ "กรอก…ที่นี่ · ตัวอย่าง: …" (ช่องหลักฐาน · ข้อสังเกต · แนวทางแก้ไข · ขอบเขต · หน่วยรับตรวจ · ข้อสรุป · ข้อเสนอแนะ · Root cause · Corrective · Preventive) · CSS: placeholder เทา #9aa4b2 ตัวเอียง · ข้อความที่กรอกดำ #111418
- ช่องหลักฐานเดิม **เติมเลขอ้างอิงคู่มือเป็นค่าจริงสีดำไว้ก่อน** ทำให้แยกไม่ออกว่าอะไรคือสิ่งที่ผู้ตรวจกรอกเอง → ย้ายไปอยู่ในคำแนะนำสีเทา ("อ้างอิงที่คาดไว้: …") ช่องเริ่มว่าง
- verify: node --check ผ่านทุกแอป · deploy ตรงขนาด (portal 44,780 / safety 265,225) · เปิดจริงทั้งโหมด view และโหมดปกติ เห็นแถบและ placeholder ตามที่ตั้งใจ · Portal ขึ้นช่อง SM และข้อความ "ปิดได้เฉพาะ SM — ยังไม่ได้กำหนดอีเมล SM"
- **26 SEP — สมัครใหม่แล้วขึ้น auth/invalid-email:** โค้ดเดิมส่งค่าจากช่องอีเมลให้ Firebase ตรง ๆ แล้วเอา `e.message` ภาษาอังกฤษมาแสดง ผู้ใช้จึงไม่รู้ว่าพิมพ์อะไรผิด · เพิ่มการตรวจฝั่งหน้าเว็บก่อนส่ง — ล้างช่องว่าง/อักขระมองไม่เห็น + ตัวพิมพ์เล็ก · ตรวจรูปแบบอีเมลพร้อมแสดง "ที่พิมพ์ไว้: …" · จับกรณีพิมพ์ด้วยแป้นไทย · ตรวจรหัสผ่านสั้นกว่า 6 ตัวก่อนกดสมัคร · แปล error code ของ Firebase เป็นไทยทั้งในแอปตรวจและ Portal (แสดง code กำกับไว้ด้วย) · ช่องอีเมลปิด autocapitalize/autocorrect ซึ่งเป็นสาเหตุที่พบบ่อยบน iPad
